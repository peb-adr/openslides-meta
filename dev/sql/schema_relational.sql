...
CREATE TABLE action_worker_t (
    ...
    state varchar(256)
        ...
        CONSTRAINT enum_action_worker_state CHECK (state IN ('running', 'end', 'aborted')),
    created timestamptz
...
CREATE TABLE agenda_item_t (
    ...
    type varchar(256)
        CONSTRAINT enum_agenda_item_type CHECK (type IN ('common', 'internal', 'hidden'))
...
CREATE TABLE assignment_t (
    ...
    phase varchar(256)
        CONSTRAINT enum_assignment_phase CHECK (phase IN ('search', 'voting', 'finished'))
...
CREATE TABLE import_preview_t (
    ...
    name varchar(256)
        ...
        CONSTRAINT enum_import_preview_name CHECK (name IN ('account', 'participant', 'topic', 'committee', 'motion')),
    state varchar(256)
        CONSTRAINT required_import_preview_state NOT NULL
        CONSTRAINT enum_import_preview_state CHECK (state IN ('warning', 'error', 'done')),
    created timestamptz
...
CREATE TABLE meeting_t (
    ...
    language varchar(256)
        CONSTRAINT enum_meeting_language CHECK (language IN ('en', 'de', 'it', 'es', 'ru', 'cs', 'fr'))
    ...
    applause_type varchar(256)
        CONSTRAINT enum_meeting_applause_type CHECK (applause_type IN ('applause-type-bar', 'applause-type-particles'))
    ...
    export_csv_encoding varchar(256)
        CONSTRAINT enum_meeting_export_csv_encoding CHECK (export_csv_encoding IN ('utf-8', 'iso-8859-15'))
    ...
    export_pdf_pagenumber_alignment varchar(256)
        CONSTRAINT enum_meeting_export_pdf_pagenumber_alignment CHECK (export_pdf_pagenumber_alignment IN ('left', 'right', 'center'))
        CONSTRAINT default_meeting_export_pdf_pagenumber_alignment DEFAULT 'center',
    export_pdf_fontsize integer
        CONSTRAINT enum_meeting_export_pdf_fontsize CHECK (export_pdf_fontsize IN (10, 11, 12))
    ...
    export_pdf_pagesize varchar(256)
        CONSTRAINT enum_meeting_export_pdf_pagesize CHECK (export_pdf_pagesize IN ('A4', 'A5'))
    ...
    agenda_numeral_system varchar(256)
        CONSTRAINT enum_meeting_agenda_numeral_system CHECK (agenda_numeral_system IN ('arabic', 'roman'))
        CONSTRAINT default_meeting_agenda_numeral_system DEFAULT 'arabic',
    agenda_item_creation varchar(256)
        CONSTRAINT enum_meeting_agenda_item_creation CHECK (agenda_item_creation IN ('always', 'never', 'default_yes', 'default_no'))
        CONSTRAINT default_meeting_agenda_item_creation DEFAULT 'default_no',
    agenda_new_items_default_visibility varchar(256)
        CONSTRAINT enum_meeting_agenda_new_items_default_visibility CHECK (agenda_new_items_default_visibility IN ('common', 'internal', 'hidden'))
    ...
    motions_default_line_numbering varchar(256)
        CONSTRAINT enum_meeting_motions_default_line_numbering CHECK (motions_default_line_numbering IN ('outside', 'inline', 'none'))
    ...
    motions_recommendation_text_mode varchar(256)
        CONSTRAINT enum_meeting_motions_recommendation_text_mode CHECK (motions_recommendation_text_mode IN ('original', 'changed', 'diff', 'agreed'))
        CONSTRAINT default_meeting_motions_recommendation_text_mode DEFAULT 'diff',
    motions_default_sorting varchar(256)
        CONSTRAINT enum_meeting_motions_default_sorting CHECK (motions_default_sorting IN ('number', 'weight'))
        CONSTRAINT default_meeting_motions_default_sorting DEFAULT 'number',
    motions_number_type varchar(256)
        CONSTRAINT enum_meeting_motions_number_type CHECK (motions_number_type IN ('per_category', 'serially_numbered', 'manually'))
    ...
    motions_amendments_text_mode varchar(256)
        CONSTRAINT enum_meeting_motions_amendments_text_mode CHECK (motions_amendments_text_mode IN ('freestyle', 'fulltext', 'paragraph'))
    ...
    motion_poll_ballot_paper_selection varchar(256)
        CONSTRAINT enum_meeting_motion_poll_ballot_paper_selection CHECK (motion_poll_ballot_paper_selection IN ('NUMBER_OF_DELEGATES', 'NUMBER_OF_ALL_PARTICIPANTS', 'CUSTOM_NUMBER'))
    ...
    motion_poll_default_onehundred_percent_base varchar(256)
        CONSTRAINT enum_meeting_motion_poll_default_onehundred_percent_base CHECK (motion_poll_default_onehundred_percent_base IN ('Y', 'YN', 'YNA', 'N', 'valid', 'cast', 'entitled', 'entitled_present', 'disabled'))
        CONSTRAINT default_meeting_motion_poll_default_onehundred_percent_base DEFAULT 'YNA',
    motion_poll_default_backend varchar(256)
        CONSTRAINT enum_meeting_motion_poll_default_backend CHECK (motion_poll_default_backend IN ('long', 'fast'))
        CONSTRAINT default_meeting_motion_poll_default_backend DEFAULT 'fast',
    motion_poll_projection_name_order_first varchar(256)
        CONSTRAINT required_meeting_motion_poll_projection_name_order_first NOT NULL
        CONSTRAINT enum_meeting_motion_poll_projection_name_order_first CHECK (motion_poll_projection_name_order_first IN ('first_name', 'last_name'))
    ...
    users_pdf_wlan_encryption varchar(256)
        CONSTRAINT enum_meeting_users_pdf_wlan_encryption CHECK (users_pdf_wlan_encryption IN ('', 'WEP', 'WPA', 'nopass'))
...
This email was generated automatically.',
    ...
    assignment_poll_ballot_paper_selection varchar(256)
        CONSTRAINT enum_meeting_assignment_poll_ballot_paper_selection CHECK (assignment_poll_ballot_paper_selection IN ('NUMBER_OF_DELEGATES', 'NUMBER_OF_ALL_PARTICIPANTS', 'CUSTOM_NUMBER'))
    ...
    assignment_poll_default_onehundred_percent_base varchar(256)
        CONSTRAINT enum_meeting_assignment_poll_default_onehundred_percent_base CHECK (assignment_poll_default_onehundred_percent_base IN ('Y', 'YN', 'YNA', 'N', 'valid', 'cast', 'entitled', 'entitled_present', 'disabled'))
        CONSTRAINT default_meeting_assignment_poll_default_onehundred_percent_base DEFAULT 'valid',
    assignment_poll_default_backend varchar(256)
        CONSTRAINT enum_meeting_assignment_poll_default_backend CHECK (assignment_poll_default_backend IN ('long', 'fast'))
        CONSTRAINT default_meeting_assignment_poll_default_backend DEFAULT 'fast',
    poll_ballot_paper_selection varchar(256)
        CONSTRAINT enum_meeting_poll_ballot_paper_selection CHECK (poll_ballot_paper_selection IN ('NUMBER_OF_DELEGATES', 'NUMBER_OF_ALL_PARTICIPANTS', 'CUSTOM_NUMBER')),
    ...
    poll_default_onehundred_percent_base varchar(256)
        CONSTRAINT enum_meeting_poll_default_onehundred_percent_base CHECK (poll_default_onehundred_percent_base IN ('Y', 'YN', 'YNA', 'N', 'valid', 'cast', 'entitled', 'entitled_present', 'disabled'))
        CONSTRAINT default_meeting_poll_default_onehundred_percent_base DEFAULT 'YNA',
    poll_default_backend varchar(256)
        CONSTRAINT enum_meeting_poll_default_backend CHECK (poll_default_backend IN ('long', 'fast'))
...
CREATE TABLE motion_change_recommendation_t (
    ...
    type varchar(256)
        CONSTRAINT enum_motion_change_recommendation_type CHECK (type IN ('replacement', 'insertion', 'deletion', 'other'))
...
CREATE TABLE motion_state_t (
    ...
    css_class varchar(256)
        ...
        CONSTRAINT enum_motion_state_css_class CHECK (css_class IN ('grey', 'red', 'green', 'lightblue', 'yellow'))
        CONSTRAINT default_motion_state_css_class DEFAULT 'lightblue',
    restrictions varchar(256)[]
        CONSTRAINT enum_motion_state_restrictions CHECK (restrictions <@ ARRAY['motion.can_see_internal', 'motion.can_manage_metadata', 'motion.can_manage', 'is_submitter']::varchar[])
    ...
    merge_amendment_into_final varchar(256)
        CONSTRAINT enum_motion_state_merge_amendment_into_final CHECK (merge_amendment_into_final IN ('do_not_merge', 'undefined', 'do_merge'))
...
CREATE TABLE organization_t (
    ...
    default_language varchar(256)
        CONSTRAINT enum_organization_default_language CHECK (default_language IN ('en', 'de', 'it', 'es', 'ru', 'cs', 'fr'))
...
CREATE TABLE poll_t (
    ...
    type varchar(256)
        ...
        CONSTRAINT enum_poll_type CHECK (type IN ('analog', 'named', 'pseudoanonymous', 'cryptographic')),
    backend varchar(256)
        CONSTRAINT required_poll_backend NOT NULL
        CONSTRAINT enum_poll_backend CHECK (backend IN ('long', 'fast'))
    ...
    pollmethod varchar(256)
        ...
        CONSTRAINT enum_poll_pollmethod CHECK (pollmethod IN ('Y', 'YN', 'YNA', 'N')),
    state varchar(256)
        CONSTRAINT enum_poll_state CHECK (state IN ('created', 'started', 'finished', 'published'))
    ...
    onehundred_percent_base varchar(256)
        ...
        CONSTRAINT enum_poll_onehundred_percent_base CHECK (onehundred_percent_base IN ('Y', 'YN', 'YNA', 'N', 'valid', 'cast', 'entitled', 'entitled_present', 'disabled'))
...
CREATE TABLE speaker_t (
    ...
    speech_state varchar(256)
        CONSTRAINT enum_speaker_speech_state CHECK (speech_state IN ('contribution', 'pro', 'contra', 'intervention', 'interposed_question')),
    answer boolean,
    note varchar(250),
...
CREATE TABLE user_t (
    ...
    organization_management_level varchar(256)
        CONSTRAINT enum_user_organization_management_level CHECK (organization_management_level IN ('superadmin', 'can_manage_organization', 'can_manage_users')),
